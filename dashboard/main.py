...
from .data.display_specifications.hcs_clean_dictionaries import FILTER_OPTIONS, FILTER_OPTIONS_METHOD, BARCHART_ALLOWED
# GLOBAL
LANGUAGE = 'EN' #'DE'
...
# Methods:
# Software tools, wordclouds, datageneration methods
...
re_c = Category20c[len(re_fields)]
...
# TODO maybe replace this at some point, but all dict from Silke are based on human readable column names
...
def select_data_corr(question, question2, data_filters, data_filters_method):
    ...
    xtype = None
    if q1_key[0] in HCS_dtypesWOmc.keys():
        xtype = HCS_dtypesWOmc[q1_key[0]]

    if xtype == 'category':
        x_range = HCS_orderedCats[q1_key[0]]
    else:
        x_range = None

    ytype = None
    if q1_key[0] in HCS_dtypesWOmc.keys():
        xtype = HCS_dtypesWOmc[q2_key[0]]
    if ytype == 'category':
        y_range = HCS_orderedCats[q2_key[0]]
    else:
        y_range = None
    ...
    df = filter_dataframe(survey_data, include=list(set(q1_key_clean+ q2_key_clean +[FILTER_BY] + method_include)), exclude=[]+method_exclude+exclude_area, exclude_nan=exclude_nan)
    
    # if list, merge somehow..., multi column crosstab


    cross_tab = calculate_crosstab(df, q1_key[0], q2_key[0])
    ...
    cross_tab['x_values'] = cross_tab[q1_key[0]]
    cross_tab['y_values'] = cross_tab[q2_key[0]]
    cross_tab['color'] = ['#A0235A' for i in cross_tab[q2_key[0]]]
...
multi_filter = pn.widgets.MultiChoice(name="Filter by data by question specific filter", value=[], 
    options=FILTER_OPTIONS_METHOD, visible=False)
...
wordcloud = generate_wordcloud(text_list, height=600, width=600)
...
wordcloud_soft = generate_wordcloud(text_list, height=600, width=600)
svg_pane_software = pn.pane.SVG(wordcloud_soft.to_svg(), width=wordcloud_soft.width, height=wordcloud_soft.height)
...
wordcloud_repo = generate_wordcloud(text_list, height=600, width=600)
svg_pane_repo = pn.pane.SVG(wordcloud_repo.to_svg(), width=wordcloud_repo.width, height=wordcloud_repo.height)
...
def update(target, event, question_sel, f_choice, m_choice, q_filter, charttype):
    ...
    #   fig = bokeh_barchart(source, y=y_keys, factors=y_keys, legend_labels=y_keys, fill_color=fill_colors, orientation='horizontal', **display_options)#'horizontal')#df.data['factors'], legend_labels='legend_labels')#, figure=figure)
    #elif charttype ==3:
    #   fig = bokeh_barchart(source, y=y_keys, factors=y_keys, legend_labels=y_keys, fill_color=fill_colors, orientation='vertical', **display_options)#df.data['factors'], legend_labels='legend_labels')#, figure=figure)
    #else:
    #    fig = None
...
# But I did not figure out how to update a full bunch of tabs
# So the update function has to be splitted to archive this somehow, maybe with some cashed data...
...
for control in controls_bar:
    control.link(fig_exp1, callbacks={'value': gen_update_exp1})
    #control.link(fig_corr, callbacks={'value': gen_update_corr})
    #control.link(fig2, callbacks={'value': gen_update_exp2})
    #control.link(fig3, callbacks={'value': gen_update_bar})#gen_update_pie})
...
# Overview part
overview_sec = pn.Column(pn.Row(fig_ov1, fig_ov2), pn.Row(fig_ov3, fig_ov4))


# Tools
row4 = pn.Column(md_text_tools_used[LANGUAGE], "## Methods:\n", svg_pane, "## Software:\n", svg_pane_software,"## Repositories:\n", svg_pane_repo)
methods_tools_sec = row4


# Question Explorer
# Merge Quesiton explorer and Correlation explorer, allow two questions sied by side?
...
inputs2 = pn.Column(*controls_bar2, width=800)
row2 =  pn.Column(pn.Row(inputs, inputs2), pn.Row(fig_exp1, fig_exp2))
# Correlation 
#inputs_corr = pn.Column(*controls_corr, width=800)
row3 = pn.Column(pn.Row(fig_corr, leg_corr))
coree_ex_sec = row3
question_ex_sec = pn.Column(row2, row3)
...
overall_accordion = pn.Accordion((accordion_titles[LANGUAGE][0], global_filters_sec),
                                ...
                                (accordion_titles[LANGUAGE][3], question_ex_sec), sizing_mode="scale_both")#,
                                #('Correlation explorer', coree_ex_sec))
overall_accordion.active = [0,1]
overall_accordion.scroll = True
overall_accordion.margin = 20
