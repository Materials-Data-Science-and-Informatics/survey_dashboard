    ...
    if key in HCS_MCList:
        ...
        for key in HCS_colnamesDict.keys():
            if key_s in key:
                if (
                    not "other" in key
                ):  # for now, needs to be included in HCS_MCsubquestions and others is strange
...
re_fields = [
    ...
    "Earth Science",
    "Engineering Science",
    "Life Science",  #'Health', 'Biology', 'Computer Science',
    "Mathematics",
    "Other",
    "Physics",
    "Psychology",
...
def select_data_wordcloud(
    data_filters, data_filters_method, content=["dataGenMetdataGenMethodSpec_hodSpec_"]
):
    """
...
question_select = pn.widgets.Select(
    name=md_text_select_widgets[0][LANGUAGE], value=START_DATA_BAR, options=questions
)  # , description="Select the survey question, which results should be displayed.")
...
question_select2 = pn.widgets.Select(
    name=md_text_select_widgets[1][LANGUAGE], value=START_DATA_BAR2, options=questions
)  # , description="Select the survey question, which results should be displayed.")
...
# Others
plot_aspect_ratio_select = pn.widgets.Select(
    name="Filter data", value="Cum. Sum", options=FILTER_OPTIONS
)
menu = [
    "Barchart",
    "Correlation Plot",
]  # ('Barchart', 'bar_chart'), ('Correlation Plot', '(corr_plot')]
...
start_corr_data, display_options_corr, marker_scale = select_data_corr(
...
start_display_data, ydata_spec, display_options = select_data(
    START_DATA_OV1, data_filters, data_filters_method
)
y_keys = ydata_spec.data["y_keys"]
fig_ov1 = pn.pane.Bokeh(
    bokeh_barchart(
        start_display_data,
        y=y_keys,
        factors=y_keys,
...
start_display_data, ydata_spec, display_options = select_data(
    START_DATA_OV2, data_filters, data_filters_method
)
y_keys = ydata_spec.data["y_keys"]
fig_ov2 = pn.pane.Bokeh(
    bokeh_barchart(
        start_display_data,
        y=y_keys,
        factors=y_keys,
...
start_display_data, ydata_spec, display_options = select_data(
    START_DATA_OV3, data_filters, data_filters_method
)
y_keys = ydata_spec.data["y_keys"]
fig_ov3 = pn.pane.Bokeh(
    bokeh_barchart(
        start_display_data,
        y=y_keys,
        factors=y_keys,
...
start_display_data, ydata_spec, display_options = select_data(
    START_DATA_OV4, data_filters, data_filters_method
)
y_keys = ydata_spec.data["y_keys"]
fig_ov4 = pn.pane.Bokeh(
    bokeh_barchart(
        start_display_data,
        y=y_keys,
        factors=y_keys,
...
text_list = select_data_wordcloud(
    data_filters, data_filters_method, content=["dataGenMethodSpec_"]
)
wordcloud = generate_wordcloud(
    text_list, height=DEFAULT_FIGURE_HEIGHT, width=ACCORDION_WIDTH
)  # half_width)
svg_pane = pn.pane.Bokeh(
    interactive_wordcloud(wordcloud),
    width=wordcloud.width,
    height=wordcloud.height + offsetheight_for_tabs,
)
...
wordcloud_soft = generate_wordcloud(
    text_list, height=DEFAULT_FIGURE_HEIGHT, width=ACCORDION_WIDTH
)  # half_width)
svg_pane_software = pn.pane.Bokeh(
    interactive_wordcloud(wordcloud_soft),
    width=wordcloud.width,
    height=wordcloud.height + offsetheight_for_tabs,
)
...
wordcloud_repo = generate_wordcloud(
    text_list, height=DEFAULT_FIGURE_HEIGHT, width=ACCORDION_WIDTH
)  # half_width)
svg_pane_repo = pn.pane.Bokeh(
    interactive_wordcloud(wordcloud_repo),
    width=wordcloud.width,
    height=wordcloud.height + offsetheight_for_tabs,
)
...
methods_tabs = pn.Column(
    ...
    pn.Tabs(
        (md_text_tools_tabs["methods"][LANGUAGE], svg_pane),
...
def update_corr(target, event, question_sel, question_sel2, f_choice, m_choice):
    ...
    leg_corr = create_legend_corr(
        fig_corr, colors=df.data["color"], scale_m=marker_scale
    )
...
def gen_update_exp1(target, event):
    (
        f_choice,
        m_choice,
    ) = multi_choice, multi_choice_method
...
overall_accordion = pn.Accordion(
    (accordion_titles[LANGUAGE][0], global_filters_sec),
